...
acoral_u8 acoral_get_drive(const acoral_char *pathname)
{
	acoral_u8 fs_ret;
	acoral_mutex_pend(fs_mutex,0);
	fs_ret=GetDrive(pathname);
	acoral_mutex_post(fs_mutex);
	return fs_ret;
}
...
acoral_32 add_file_driver(void *devname)
{
	acoral_u32 ret;
	ret=acoral_mutex_pend(fs_mutex,0);
	if(ret!=MUTEX_SUCCED)
		return -1;
	AddFileDriver(FsCommand,(acoral_char *)devname);
	acoral_mutex_post(fs_mutex);
	return 0;
}

acoral_32 remove_file_driver(acoral_u8 Drive)
{
	acoral_u32 ret;
	ret=acoral_mutex_pend(fs_mutex,0);
	if(ret!=MUTEX_SUCCED)
		return -1;
	RemoveFileDriver(Drive);
	acoral_mutex_post(fs_mutex);
	return 0;
}
